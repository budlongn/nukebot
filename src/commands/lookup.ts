...
export async function lookup(args: string[], message: Message) {
    ...
    const resp = await client.wow.character(['progression', 'talents', 'items'], {origin: 'us', realm: args[0], name: args[1]})
    const character: Character = resp.data
    const spec: Spec = find(character.talents, (x: Talents) => {return x.selected}).spec
